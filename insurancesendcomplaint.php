...
if (isset($_POST['send'])) 
{
	...
	$r=insert($q);
	return redirect("insurancesendcomplaint.php");
...
<?php 
$q="SELECT * FROM complaints INNER JOIN   `insurance_company` ON `insurance_company`.`login_id`=`complaints`.`sender_id` where login_id='$loginid' ";
$r=select($q);
$slno=1;
foreach ($r as $row) {
?>
